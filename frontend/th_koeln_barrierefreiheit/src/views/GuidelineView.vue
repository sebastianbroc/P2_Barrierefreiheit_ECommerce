  ...
  methods: {
    ...
    getAnnotation(input){
      ...
      if(this.activeAnnotation && this.activeAnnotation.id === parseInt(input.srcElement.id)){
        ...
        document.addEventListener("scroll", this.handleClick)
        this.timeOfAnnotationOpen = moment();
        this.handleVoteButtonColor()
      }
    },
    handleAnnotationVote(input){
      if(input.srcElement.id === "upvote"){
        if(this.activeAnnotation.userVote === "u"){
          //remove the previously entered upvote
          this.activeAnnotation.score--;
          this.activeAnnotation.userVote = null
          //TODO: do stuff with the api
        } else {
          if(this.activeAnnotation.userVote === "d"){
            this.activeAnnotation.score+=2;
          } else {
            this.activeAnnotation.score++;
          }

          this.activeAnnotation.userVote = "u"
          //TODO: do stuff with the api
        }
      } else if (input.srcElement.id === "downvote"){
        if(this.activeAnnotation.userVote === "d") {
          //remove the previously entered downvote
          this.activeAnnotation.score++;
          this.activeAnnotation.userVote = null
          //TODO: do stuff with the api
        } else {
          ...
          this.activeAnnotation.userVote = "d"
          //TODO: do stuff with the api
        }
      }
  ...
  #annotation {
    ...
    .annotation_voting {
      ...
      #upvote {
        ...
        &:hover, &.active {
          border-bottom-color: $mi-pink;
        }
      }
      #downvote {
        border-top: $bfs-xs solid white;
        &:hover, &.active {
          border-top-color: $mi-pink;
        }
      }
    }
